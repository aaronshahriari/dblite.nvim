...
**Breaking a command across lines.** A long document argument on one line is unreadable, so a command continues onto the next line in two cases:

```redis
# a quote left open — the newline is part of the value
JSON.SET doc:1 $ '{
  "name": "widget",
  "qty": 2
}'

# a trailing backslash, which is dropped
HSET user:1042   email a@example.com   name Aaron
```

`run at` from **any** line of a continued command runs all of it, and a visual range grows to whole commands rather than cutting one in half. Outside quotes, `#` starts a comment and a trailing comment is stripped before the command is sent; inside a quoted value both are ordinary data, so `SET colour "#ff0000"` and a `#` inside multi-line JSON are safe.
...
**Sizing.** `split_size.width` and `.height` take either a cell count or a **fraction of the screen** — `0.5` is half the terminal, whatever width it happens to be. Values are clamped so the editor always keeps a usable window: a `width` wider than the screen would otherwise squeeze the editor down to `'winwidth'`, which looks like a layout bug rather than a configuration mistake.

Your config is authoritative. A size you drag to by hand is remembered for the rest of the session — so toggling dbout away and back keeps it — but it is deliberately **not** written to disk, and it does not outlive the session. Persisting it meant a size on disk silently outranked `split_size` in your config; since it was saved on every toggle rather than only on a real resize, one toggle was enough to freeze the configured value for good and editing it appeared to do nothing.
...
require('dblite').setup({
  ...
  split_size     = { width = 0.4, height = 20 }, -- whole number = cells, decimal < 1 = fraction
                                                 ...
                                                 -- usable. A size you drag to is kept for the
                                                 -- session only — config stays authoritative.
